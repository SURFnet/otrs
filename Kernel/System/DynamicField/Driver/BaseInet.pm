...
package Kernel::System::DynamicField::Driver::BaseInet;
...
sub SearchSQLGet {
    ...
    $Kernel::OM->Get('Kernel::System::Log')->Log(
        'Priority' => 'debug',
        'Message'  => "Found Operator $Param{Operator}",
    );

    if ( $Param{Operator} eq 'Like' ) {
        return " '$Param{SearchTerm}' << $Param{TableAlias}.value_inet ";
        my $SQL = $Kernel::OM->Get('Kernel::System::DB')->QueryCondition(
            Key   => "$Param{TableAlias}.value_inet",
    ...
    if ( $Param{Operator} eq 'Empty' ) {
        if ( $Param{SearchTerm} ) {
            return " $Param{TableAlias}.value_inet IS NULL ";
        }
        else {
            my $DatabaseType = $Kernel::OM->Get('Kernel::System::DB')->{'DB::Type'};
            if ( $DatabaseType eq 'oracle' ) {
                return " $Param{TableAlias}.value_inet IS NOT NULL ";
            }
            else {
                return " $Param{TableAlias}.value_inet <> '' ";
            }
        }
    }
    ...
    my $SQL = " $Param{TableAlias}.value_inet $Operators{ $Param{Operator} } ";
    $SQL .= "'$Param{SearchTerm}'";
